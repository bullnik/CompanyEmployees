namespace CompanyEmployees.Models.Emploees
{
    public class DataGenerator : IDataGenerator
    {
        ...
        public IEnumerable<Employee> GenerateEmployee(int count)
        {
            for (int i = 0; i < count; i++)
            {
                yield return GenerateEmployee();
            }
        }

        public Vacation GenerateVacation(Employee employee, int duration)
        {
            DateTime dateTime = new DateTime(DateTime.Now.Year, 1, 1) + TimeSpan.FromDays(_random.Next(365));
            return new(dateTime, dateTime + TimeSpan.FromDays(duration), employee);
        }
